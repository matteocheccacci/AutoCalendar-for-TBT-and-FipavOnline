...
function syncGmailToSheetAndCalendar(customQuery) {
  ...
  threads.forEach(thread => {
    ...
    messages.forEach(msg => {
      ...
      if (apiKey) dataGara = callGeminiAI(plainBody, apiKey);
...
function callGeminiAI(text, key) {
  ...
  const prompt = `Analizza questa email di designazione pallavolo. Estrai i dati. Rispondi SOLO con un JSON valido: {"data":"DD/MM/YYYY","ora":"HH:MM","luogo":"...","squadraCasa":"...","squadraOspite":"...","categoria":"...","numeroGara":"...","codA":"...","codF":"...","arb1":"...","arb2":"..."}. Testo: ${text}`;
  ...
  try {
    ...
    for (var i = 0; i < lines.length; i++) {
      if (lines[i].includes(" - ") && !lines[i].toLowerCase().includes("girone") && !lines[i].toLowerCase().includes("gara")) {
        var lastDash = lines[i].lastIndexOf(" - ");
        res.squadraCasa = lines[i].substring(0, lastDash).trim();
        res.squadraOspite = lines[i].substring(lastDash + 3).split(/del \d|alle|ore/i)[0].trim();
        break;
      }
    }
